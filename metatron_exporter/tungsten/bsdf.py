...
from ..metatron.linalg import to_vec2, to_vec3, to_vec4, vecable, isvecable, cast
...
def import_bsdf(json, name: str = "") -> str:
    ...
    mat_type = json["type"]
    if mat_type == "lambert":
        reflectance = import_texture(
            json["albedo"], mat_name + "/reflectance", Texture_Attribute()
        )
        mat = compo.Physical_Material(reflectance=reflectance)
    elif mat_type == "dielectric" or mat_type == "rough_dielectric":
        eta = import_texture(
            json["ior"], mat_name + "/eta", Texture_Attribute(color=False)
        )
        alpha = (
            import_texture(
                json["roughness"], mat_name + "/alpha", Texture_Attribute(vector=True)
            )
        ...
        if "material" in json:
            ...
            k = import_texture(
                json["k"], mat_name + "/k", Texture_Attribute(color=False)
            )
        alpha = (
            import_texture(
                json["roughness"], mat_name + "/alpha", Texture_Attribute(vector=True)
    ...
    materials[mat_path] = compo.json(
        entity=mat_path, type=type(mat).__name__.lower(), serialized=mat
    )
    return mat_path
